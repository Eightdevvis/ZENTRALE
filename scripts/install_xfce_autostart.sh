...
#
# 4. Schreibt ~/.xsessionrc: Bildschirm-Modus (aussenposten_bildschirm.py)
#    und Blanking AUS (kein X-Screensaver, kein DPMS — der Wandmonitor bleibt
#    an). ACHTUNG Debian: /etc/X11/Xsession.d/40x11-common_xsessionrc liest
#    ~/.xsessionrc — ~/.xprofile liest NIEMAND (das ist GDM/Ubuntu-Sitte).
#    Bis 2026-09-14 stand das alles in ~/.xprofile und lief deshalb nie: der
#    Pi blankte mit den X-Defaults nach 10 min, obwohl der Installer
#    scheinbar etwas anderes gesetzt hatte.
#
...
# --- ~/.xsessionrc: Bildschirm-Modus setzen + Blanking aus ------------------
# Auf Pi 3 B + Bookworm wird der EDID-preferred Mode des Monitors nicht
# automatisch von fkms uebernommen. Ohne Nachhilfe landen wir in 1024x768
# statt der nativen Aufloesung -> verschobenes Bild, evtl.
# Pixel-Format-Mismatch (pinker Rand).
#
# Frueher stand hier ein hart verdrahtetes `xrandr --output HDMI-1 --mode
# 1920x1080`. Das ist zweimal falsch gegangen: der Ausgang heisst nicht auf
# jedem Knoten HDMI-1, und ein Monitor, der 1080p gar nicht anbietet, laesst
# den Aufruf ins Leere laufen — mit `|| true` sogar lautlos. Genau so stand
# der Pi auf 1024x768 an einem Ultrawide und zog das Bild in die Breite.
#
# Jetzt fragt scripts/aussenposten_bildschirm.py den Monitor selbst: welche
# Modi bietet er an, und wie gross ist er physisch? Gewaehlt wird der Modus,
# dessen Seitenverhaeltnis am besten zum Panel passt, bei Gleichstand der mit
# den meisten Pixeln.
#
# .xsessionrc wird von Debians Xsession (40x11-common_xsessionrc) beim
# X-Login VOR xfce4-session gelesen, damit Apps direkt in der korrekten
# Aufloesung spawnen. NICHT .xprofile: die Datei liest auf Debian/lightdm
# niemand — genau daran ist das Blanking-Setting bis 2026-09-14 gescheitert.
rm -f "$HOME/.xprofile"
cat > "$HOME/.xsessionrc" << 'EOF'
# Auto-generated by install_xfce_autostart.sh.
# Bildschirm-Modus passend zum angeschlossenen Panel (siehe Kommentar im
# Installer). Faellt auf den alten fest verdrahteten Aufruf zurueck, falls
# das Skript (noch) nicht da ist.
if [ -x /opt/zentrale/scripts/aussenposten_bildschirm.py ]; then
  /opt/zentrale/scripts/aussenposten_bildschirm.py || true
else
  xrandr --output HDMI-1 --mode 1920x1080 --rate 60 || true
fi

# Bildschirm-Blanking AUS: kein X-Screensaver, kein DPMS. Der Monitor an
# der Wand bleibt an — ZENTRALE soll einen ansprechen koennen, wenn man
# vorbeigeht, und ein schwarzer Schirm ist dafuer das Gegenteil. Die
# X-Defaults (Blank + DPMS-Off nach 600 s) waren der Grund, warum der Pi
# nach 10 min dunkel wurde. Nachts abschalten kann spaeter der Wachplan
# (memory/betrieb/wachplan.md) uebernehmen — dann gezielt, nicht per Timer.
xset s off || true
xset s noblank || true
xset -dpms || true
EOF
...
EOF
    ...
    #
    # Die drei xset vorneweg sind Belt-and-Suspenders zu ~/.xsessionrc: der
    # Autostart laeuft garantiert IN der X-Session, egal welcher Display-
    # Manager welche Datei liest. Monitor bleibt an (siehe .xsessionrc oben).
    cat > "$AUTOSTART_DIR/zentrale.desktop" << EOF
[Desktop Entry]
Type=Application
Name=ZENTRALE Kiosk (TUI)
Comment=Maximiertes xterm mit der curses-TUI gegen das PC-Backend (KI-frei, browserlos)
Exec=bash -c 'xset s off; xset s noblank; xset -dpms; while true; do xterm -maximized -u8 -fa Monospace -fs ${TUI_FONTSIZE} -bg black -fg white -e bash -c "export TERM=xterm-256color; export ZENTRALE_URL=${BACKEND_URL}; cd /opt/zentrale && exec python3 tui/zentrale_tui.py"; sleep 2; done'
X-GNOME-Autostart-enabled=true
EOF
