...
> + Lizenzregeln stehen in **`memory/maps/maps_quellen.md`** (Provenienz-Charta).
...
- `trade` — **erster gebauter Overlay (2026-06)**, Komposit aus Sub-Layern, je
  EINE offizielle Quelle (siehe `memory/maps/maps_quellen.md`). Live: `routes`
...
Quellen-Recherche (liveuamap & Co.) → Kernbefunde in `memory/maps/maps_quellen.md`. Ergebnis:
...
- ~~**Konflikt-/Overlay-Datenquelle offline**~~ **→ GELÖST (Schritt 2):** UCDP GED
  ...
  oben + in `memory/maps/maps_quellen.md`. Liveuamap selbst ist online — eigene Quelle/Format
  nötig.
